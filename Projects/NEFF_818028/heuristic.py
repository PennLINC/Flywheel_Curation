### This heuristic organizes NEFF (818028) data on Flywheel
###
### Ellyn Butler
...
def gather_session_indeces():
    ...
    cs = fw.projects.find_first('label="{}"'.format("NEFF_818028"))
    ...
    for x in range(len(cs_subjects)):
        ...
        if sess_list:
            ...
            for i, y in enumerate(sess_list):
                sess_dict[y.label] = "NEFF" + str(i + 1) #!!!!!!!!
...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        # t1
        if "mprage" in protocol and 'nav' not in protocol and 'moco' in protocol and s.TR == 1.85 and s.dim1 == 256 and "DERIVED" not in s.image_type: #need to clear BIDS for this to work
            # What is up with dim1 == 484??
            info[t1w_moco].append(s.series_id)
        elif "mprage" in protocol and s.TR == 1.82 and s.dim1 == 256 and "DERIVED" not in s.image_type:
